...
/// SM3 as specifed in [GB/T 32905-2016].
///
/// SM3 is a cryptographic hash function with a 256-bit output, defined in GB/T
/// 32905-2016.
pub static SM3: Algorithm = Algorithm {
    output_len: SM3_OUTPUT_LEN,
    chaining_len: SM3_OUTPUT_LEN,
    block_len: 512 / 8,
    len_len: 64 / 8,
    block_data_order: sm3::block_data_order,
    format_output: sha256_format_output,
    initial_state: [
        u32x2!(0x7380166fu32, 0x4914b2b9u32),
        u32x2!(0x172442d7u32, 0xda8a0600u32),
        u32x2!(0xa96f30bcu32, 0x163138aau32),
        u32x2!(0xe38dee4du32, 0xb0fb0e4eu32),
        0, 0, 0, 0,
    ],
    id: AlgorithmID::SM3,
};
...
pub const SM3_OUTPUT_LEN: usize = 256 / 8;
...
pub mod test_util {
    ...
    pub static ALL_ALGORITHMS: [&'static digest::Algorithm; 6] = [
        &digest::SHA1,
        &digest::SHA256,
        &digest::SHA384,
        &digest::SHA512,
